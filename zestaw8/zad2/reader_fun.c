...
void dispatch_threads(int num_threads, char *filename, int num_records, char *pattern, int type, int variant, int signo)
{
    ...
    switch(type) {
        case 1:
            threads_read_async(num_threads, input_file, num_records, pattern, variant, signo);
            break;
        case 2:
            threads_read_sync(num_threads, input_file, num_records, pattern, variant, signo);
            break;
        case 3:
            threads_read_detached(num_threads, input_file, num_records, pattern, variant, signo);
            break;
        default:
...
struct thread_info *init_thread_info(struct search_info *info, pthread_t *tids, int num_threads, pthread_mutex_t *mutex, int variant, int signo)
{
    ...
    thread_info->variant = variant;
    thread_info->signo = signo;
    return thread_info; 
}
...
void threads_read_async(int num_threads, FILE *input_file, int num_records, char *pattern, int variant, int signo)
{
    ...
    struct thread_info *thread_info = init_thread_info(search_info, tids, num_threads, async_mutex, variant, signo);
    ...
    switch(variant) {
        ...
        case 3:
            ...
            break;
        case 4:
            pthread_kill(tids[0], signo);
            break;
        case 5:
            pthread_kill(tids[0], signo);
            break;
        default:
            fprintf(stderr, "%s\n", "FATAL ERROR! Unkown choice in switch statement");
    }
    for(int i = 0; i < num_threads; i++) {
...
{
    printf("PID %d, TID %lu got signal %s.", getpid(), pthread_self(), strsignal(signo));
}
...
void *async_search(void *arg)
{
    ...
    switch(info->variant) {
        case 3:
            signal(info->signo, handle_sig);
            break;
        case 4:
            sigemptyset(&set);
            sigaddset(&set, info->signo);
            pthread_sigmask(SIG_BLOCK, &set, NULL);
            break;
        case 5:
            signal(info->signo, handle_sig);
...
void threads_read_sync(int num_threads, FILE *input_file, int num_records, char *pattern, int variant, int signo)
{
    pthread_t *tids = malloc(MAX_THREADS * sizeof(pthread_t));
    memset(tids, 0, MAX_THREADS * sizeof(pthread_t));
...
void threads_read_detached(int num_threads, FILE *input_file, int num_records, char *pattern, int variant, int signo)
{
    pthread_t *tids = malloc(MAX_THREADS * sizeof(pthread_t));
    memset(tids, 0, MAX_THREADS * sizeof(pthread_t));
